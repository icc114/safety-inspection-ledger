...
private fun testEndpoint(raw:String):String?=runCatching{val endpoint=raw.trim().trimEnd('/');require(endpoint.startsWith("https://")||endpoint.startsWith("http://")){"地址必须以 http:// 或 https:// 开头"};val request=Request.Builder().url("$endpoint/api/health").get().build();val client=OkHttpClient.Builder().connectTimeout(12,TimeUnit.SECONDS).readTimeout(18,TimeUnit.SECONDS).build();client.newCall(request).execute().use{response->if(!response.isSuccessful)error("服务器返回 ${response.code}")}}.exceptionOrNull()?.message
