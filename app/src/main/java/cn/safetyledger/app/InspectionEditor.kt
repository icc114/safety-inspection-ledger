...
fun InspectionEditor(record:InspectionEntity,templateItems:List<TemplateItemEntity>,dao:LedgerDao,onSaved:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(14.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  item{Text("现场照片",style=MaterialTheme.typography.titleMedium);MediaActions(record.id,MediaKind.SITE,record.location,dao){message=it};Text("电子签名",style=MaterialTheme.typography.titleMedium);Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){OutlinedButton(onClick={signatureKind=MediaKind.SIGNATURE_INSPECTOR_1}){Text("检查人1")};OutlinedButton(onClick={signatureKind=MediaKind.SIGNATURE_INSPECTOR_2}){Text("检查人2")};OutlinedButton(onClick={signatureKind=MediaKind.SIGNATURE_INSPECTEE}){Text("被检查人")}};if(message.isNotBlank())Text(message)}
  item{Button(onClick={scope.launch{val failed=drafts.any{it.result==ItemResult.FAIL};dao.saveInspection(record.copy(conclusion=conclusion,rectificationAdvice=advice,responsiblePerson=responsible,deadline=deadline,status=if(failed)RecordStatus.PENDING else RecordStatus.COMPLETE,updatedAt=System.currentTimeMillis()));dao.saveInspectionItems(drafts.map{InspectionItemEntity(java.util.UUID.randomUUID().toString(),record.id,it.source.id,it.source.category,it.source.content,it.source.standard,it.result,it.problem)});CloudSyncScheduler.enqueue(context);onSaved()}},enabled=drafts.isNotEmpty()&&drafts.none{it.result==ItemResult.FAIL&&it.problem.isBlank()},modifier=Modifier.fillMaxWidth()){Text("\u4fdd\u5b58\u68c0\u67e5\u8bb0\u5f55")};Spacer(Modifier.height(24.dp))}
 }
 signatureKind?.let{kind->SignatureDialog(record.id,kind,when(kind){MediaKind.SIGNATURE_INSPECTOR_1->"检查人1签名";MediaKind.SIGNATURE_INSPECTOR_2->"检查人2签名";else->"被检查人签名"},dao,{signatureKind=null}){message=it}}
}
