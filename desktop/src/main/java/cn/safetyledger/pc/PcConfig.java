...
/** Stores the sync password encrypted with a random key that remains local to this Windows profile. */
public final class PcConfig {
    ...
    public String endpoint="", space="safety-ledger", password="", archiveRoot=Path.of(System.getProperty("user.home"),"安全检查台账资料库").toString();
    public String deviceId=UUID.randomUUID().toString(), deviceName=defaultName();

    public static PcConfig load(){
        PcConfig c=new PcConfig();try{Files.createDirectories(HOME);if(!Files.isRegularFile(FILE))return c;Properties p=new Properties();try(InputStream in=Files.newInputStream(FILE)){p.load(in);}c.endpoint=p.getProperty("endpoint","");c.space=p.getProperty("space","safety-ledger");c.archiveRoot=p.getProperty("archiveRoot",c.archiveRoot);c.deviceId=p.getProperty("deviceId",c.deviceId);c.deviceName=p.getProperty("deviceName",defaultName());String secret=p.getProperty("password","");if(!secret.isBlank())c.password=decrypt(secret);return c;}catch(Exception ignored){return c;}
    }
    public void save()throws Exception{Files.createDirectories(HOME);Properties p=new Properties();p.setProperty("endpoint",endpoint);p.setProperty("space",space);p.setProperty("archiveRoot",archiveRoot);p.setProperty("deviceId",deviceId);p.setProperty("deviceName",deviceName);p.setProperty("password",password.isBlank()?"":encrypt(password));try(OutputStream out=Files.newOutputStream(FILE)){p.store(out,"Safety Ledger PC config");}}
